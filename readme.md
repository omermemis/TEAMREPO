...
## III. Discussion
In both CMPC and DMPC, the predictive horizon *Hp* is chosen to be 25 and control horizon *Hu* is chosen to be 12. *Hp* will be chosen such that the vehicle's dynamic will be covered by the whole predicitve horizon and *Hu* should not exceed *Hp*. Normally, we can have a large control horizon, but considering the model used by MPC is an approximated linear model, we decide to choose *Hu* to be about a half of *Hp*. For comparison reason, the sample time for both CMPC and DMPC is 0.2 *s*. Note that the sample time for DMPC can be chosen smaller, such as 0.1 *s*, because as we can see later that DMPC needs less computation time campared to CMPC.
